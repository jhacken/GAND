from utils_v1.analyse_attribution_and_probability_helperFunctions import plot_visualisations
import argparse
...
def identify_overlapping_salient_words(d_saliency_dictionaries: Dict) -> Dict:
    ...
    for target_language in d_saliency_dictionaries:
        ...
        for index, individual_saliency_dict_1 in saliency_dictionary.items():
            salient_words_1 = list(source_token for source_token in individual_saliency_dict_1["salient_data"].keys())
            d_all_saliency_words[target_language].extend(salient_words_1)
            salient_pos_1 = list(pos for score, pos in individual_saliency_dict_1["salient_data"].values())
            d_all_saliency_pos[target_language].extend(salient_pos_1)
    ...
    for salient_word, salient_pos in zip(d_all_saliency_words[target_language_1], d_all_saliency_pos[target_language_1]):
        ...
        if salient_word in d_all_saliency_pos[target_language_2]:
...
def get_prob_difference(data: pd.DataFrame, d_saliency_dictionaries: Dict) -> Dict:
    ...
    """
    d_prob_difference_lists = {}

    for target_language in d_saliency_dictionaries:
        d_prob_difference_lists[target_language] = []
        ...
        for idx, row in data.iterrows():
            ...
            if str_idx in saliency_dict:
                ...
                if individual_saliency_dict["salient_data"]:  # Only skip if no salient data
                    ...
                    d_prob_difference_lists[target_language].append({
                        "index": idx,
                        "referent": referent,
    ...
    d_prob_difference_dfs = {lang: pd.DataFrame(d_prob_difference_lists[lang]) for lang in d_prob_difference_lists}

    # Print statistics
    proceed_with_visualisation = True

    for target_language in d_prob_difference_dfs:
        prob_difference_df = d_prob_difference_dfs[target_language]
    ...
    if proceed_with_visualisation:
        plot_visualisations(d_prob_difference_dfs)
    else:
        print("Cannot create plots - one or both dataframes are empty")

    return d_prob_difference_dfs
...
def main(languages: str) -> None:
    ...
    # Get probability differences and create visualisations
    _ = get_prob_difference(gand_contrastive, d_salient_words_per_target_language)
