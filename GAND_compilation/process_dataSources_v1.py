...
def perform_step_2(dataset_name: str, n_texts_to_analyse: int) -> Tuple[IterableDataset, int]:
    ...
    if dataset_name == "OpenSubtitles":
        dataset, len_dataset = load_open_subtitles_data(dataset_name, n_texts_to_analyse)
    
    return dataset, len_dataset


def perform_step_3(
        dataset_name: str, list_of_referents: List, dataset: IterableDataset, len_dataset: int,
    ...
    dict_sentences_per_referent = filter_referent_entities(
        dataset_name, list_of_referents, dataset, len_dataset,
...
def perform_step_4(
    ...
    dict_sentences_filtered_out = filter_coreference(
        dataset_name, dict_sentences_per_referent, device, max_n_sentences_per_referent
    )
...
def perform_step_5(
        dataset_name: str, list_of_referents: List, dict_sentences_filtered_out: Dict, d_referents_to_source: Dict,
        d_meta: Dict
) -> None:
    ...
    sentences_filtered_out_to_txt(
        os.path.join("output_v1", "sentences_filtered_out", dataset_name), dict_sentences_filtered_out
    )
    merge_data_per_item_into_single_xlsx(
        dataset_name, list_of_referents, d_referents_to_source, d_meta,
        os.path.join("temp_v1", "sentences_selected_perItem", dataset_name),
...
def main(
        ...
        max_n_sentences_per_referent: int
) -> None:
    ...
    dict_sentences_per_referent = perform_step_3(
        dataset_name, list_of_referents, dataset, len_dataset, n_texts_to_analyse, min_sentence_length, max_sentence_length
    )
    ...
    perform_step_5(dataset_name, list_of_referents, dict_sentences_filtered_out, d_referents_to_source, d_meta)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Device on which the script should be run."
    )
    parser.add_argument(
        "-t", "--n_texts_to_analyse",
        default=10000, type=int,
        help="Number of texts from the dataset to be analysed."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Maximum number of sentences per referent entity (i.e. person) to be selected."
    )
